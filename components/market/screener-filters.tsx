...
import {ChevronDown, X, Filter, Zap, ArrowUpRight, ArrowDownRight, BarChart3} from "lucide-react"
...
import type { AssetClass } from "@/components/data/market-data"
...
const PRESETS: Record<string, { label: string; filters: Record<string, any> }[]> = {
  stock: [
    { label: "High Growth", filters: { sector: ["Technology"], sort: "gainers" } },
    { label: "Value Stocks", filters: { sector: ["Financial Services", "Healthcare"] } },
  ],
  bitcoin: [
    { label: "Top Gainers", filters: { sort: "gainers" } },
    { label: "Most Liquid", filters: { sort: "volume" } },
  ],
  options: [
    { label: "Near Expiry", filters: { sort: "gainers" } }, // Logic handled in parent
  ],
  bond: [
    { label: "Govt Bonds", filters: { type: ["Government"] } },
    { label: "Corporate", filters: { type: ["Corporate"] } },
  ]
}

export function ScreenerFilters({ assetClass, onFiltersChange }: ScreenerFiltersProps) {
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest">
            <Zap className="h-3 w-3" /> Quick Sort
          </div>
          <div className="grid grid-cols-1 gap-2">
            <Button variant="outline" size="sm" onClick={() => updateFilters("sort", "gainers")} className="justify-start h-9">
              <ArrowUpRight className="mr-2 h-4 w-4 text-emerald-500" /> Top Gainers
            </Button>
            <Button variant="outline" size="sm" onClick={() => updateFilters("sort", "losers")} className="justify-start h-9">
              <ArrowDownRight className="mr-2 h-4 w-4 text-rose-500" /> Top Losers
            </Button>
            <Button variant="outline" size="sm" onClick={() => updateFilters("sort", "volume")} className="justify-start h-9">
              <BarChart3 className="mr-2 h-4 w-4 text-blue-500" /> Highest Volume
            </Button>
          </div>
        </div>
        ...
        {/* Example Category Filter - Stocks */}
        {assetClass === "stock" && (
            <div className="space-y-4">
              <Label className="text-xs font-bold uppercase text-muted-foreground">Sectors</Label>
              {["Technology", "Financial Services", "Healthcare", "Consumer Cyclical"].map(sector => (
                  <div key={sector} className="flex items-center space-x-2">
                    <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          updateFilters("sector", checked ? [...current, sector] : current.filter((s: string) => s !== sector))
                        }}
                    />
        ...
        {Object.keys(filters).length > 0 && (
            <Button variant="ghost" className="w-full text-rose-500 hover:text-rose-600 hover:bg-rose-50" onClick={clearFilters}>
