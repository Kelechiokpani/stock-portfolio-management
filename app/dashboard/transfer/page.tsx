...
export default function TransfersPage() {
  ...
  const sentTransfers = transfers.filter((t:any) => t.fromUserId === user.id)
  const receivedTransfers = transfers.filter((t:any) => t.toUserId === user.id)
                  ...
                  <div className="space-y-3">
                    {sentTransfers.map((transfer: any) => {
                      const portfolio = allPortfolios.find(
                          (p: any) => p.id === transfer.portfolioId
                      )
                      return (
