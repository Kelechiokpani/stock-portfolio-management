...
import { mockStocks, mockStockMarkets, type Stock } from "@/lib/mock-data"
...
export default function NewPortfolioPage() {
  ...
  function updateShares(stockId: string, shares: number) {
    setSelectedStocks(
      selectedStocks.map((s) => (s.id === stockId ? { ...s, shares: Math.max(1, shares) } : s))
    )
  }

  const isStockSelected = (stockId: string) => selectedStocks.some((s) => s.id === stockId)
  const totalValue = selectedStocks.reduce((sum, s) => sum + s.price * s.shares, 0)
