        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Badge
                variant="outline"
                className="rounded-full px-3 py-0 text-[10px] uppercase tracking-tighter border-slate-300 dark:border-slate-700"
              >
                System Admin
              </Badge>
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight font-serif italic">
              Regional Core
            </h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Configure localization parameters and demographic taxonomies.
            </p>
          </div>
          <div className="flex gap-3">
            <Button
              variant="outline"
              className="border-slate-200 dark:border-slate-800 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800"
            >
              Export Config
            </Button>
              ...
              <CardContent className="pt-6 space-y-4">
                {GENDER_OPTIONS.map((gender) => (
                  ...
                  >
                    ...
                    <Switch
                      defaultChecked
                      className="data-[state=checked]:bg-slate-900 dark:data-[state=checked]:bg-emerald-500"
                    />
                  </div>
                ))}
                ...
                <Button
                  variant="ghost"
                  className="w-full text-xs font-bold border border-dashed border-slate-300 dark:border-slate-700 h-12 hover:bg-slate-50 dark:hover:bg-slate-800"
                >
            ...
            <Card className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-none shadow-2xl">
              <CardContent className="p-6">
                ...
                <p className="text-xs opacity-70 mb-4 leading-relaxed">
                  Ensure all regional settings align with GDPR and local data
                  residency laws.
                </p>
                <Button
                  variant="secondary"
                  className="w-full text-xs bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/20 border-none"
                >
                    ...
                    >
                      ...
                      <ChevronRight
                        className={`w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity ${
                          selectedCountry === country ? "opacity-100" : ""
                        }`}
                      />
                    </button>
                  ))}
              ...
              <div className="flex-1 flex flex-col bg-white dark:bg-transparent">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 italic">
                      Operational regions and city switches.
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    className="text-[10px] h-8 dark:border-slate-700"
                  >
                ...
                <ScrollArea className="flex-1 p-6">
                  <div className="grid gap-8">
                    {Object.entries(locationData[selectedCountry]).map(
                      ([state, cities]: [string, any]) => (
                        <div key={state} className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {cities.map((city: string) => (
                              <div
                                key={city}
                                className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/50 hover:border-slate-300 dark:hover:border-slate-600 transition-all group"
                              >
                                <div className="flex items-center gap-3">
