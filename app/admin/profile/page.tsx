...
export default function ProfilePage() {
  ...
  // Accessing flags from data.user
  const isVerified =
    data.user.settings?.kycStatus === "verified" || data.user.kycVerified;
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-10 animate-in fade-in duration-700">
      ...
      <header className="relative flex flex-col md:flex-row items-center gap-8 border-b border-border/40 pb-10">
        <div className="relative group">
          <div className="h-32 w-32 rounded-full overflow-hidden border-4 border-background shadow-2xl ring-1 ring-primary/20 bg-muted flex items-center justify-center">
        ...
        <div className="flex-1 text-center md:text-left space-y-2">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
            ...
            <Badge
              variant="secondary"
              className="bg-primary/5 text-primary border-primary/10 px-3 uppercase text-[10px] font-black"
            >
              {data.user.settings?.accountType || "Individual"}
            </Badge>
            <Badge
              variant="outline"
              className="border-primary/20 text-xs capitalize font-medium"
            >
              {data.user.role}
            </Badge>
          </div>
          <div className="flex flex-wrap justify-center md:justify-start gap-4 text-muted-foreground text-sm">
            <span className="flex items-center gap-1.5">
              <Mail className="h-4 w-4" /> {data.user.email}
            </span>
            <span className="flex items-center gap-1.5">
              <Globe className="h-4 w-4" /> {data.user.profile?.country}
            </span>
          </div>
        </div>
        ...
        <Card className="lg:col-span-2 border-muted/40 bg-card/30 backdrop-blur-sm">
          ...
          <CardContent className="grid sm:grid-cols-2 gap-8 pt-8">
            ...
            <InfoItem
              icon={<Phone />}
              label="Mobile Phone"
              value={data.user.profile?.phoneNumber || "Not provided"}
            />
            <InfoItem
              icon={<Globe />}
              label="Nationality"
              value={data.user.profile?.country}
            />
            <InfoItem
              icon={<MapPin />}
              label="Residential Address"
              value={data.user.profile?.address}
        ...
        <div className="space-y-8">
            ...
            <CardContent className="space-y-5">
              <div className="flex items-center justify-between border-b border-border/40 pb-3">
                ...
                <Badge className="bg-primary/10 text-primary border-none capitalize font-bold">
                  {data.user.settings?.riskTolerance}
                </Badge>
              </div>
              <div className="flex items-center justify-between border-b border-border/40 pb-3">
                <span className="text-sm text-muted-foreground">
          ...
          <Card className="border-none bg-gradient-to-br from-secondary/50 to-background shadow-inner">
            ...
            <CardContent className="space-y-4">
              <StatusBadge
                label="Identity (KYC)"
                status={data.user.settings?.kycStatus || "pending"}
                icon={
                  isVerified ? (
              ...
              <StatusBadge
                ...
                status={data.user.agreementSigned ? "signed" : "pending"}
                icon={<FileText className="h-4 w-4" />}
              />
              <StatusBadge
                label="Account Status"
                status={data.user.accountStatus || "pending"}
...
function StatusBadge({
  ...
  const isApproved = ["verified", "signed", "approved", "active"].includes(
    status?.toLowerCase()
  );
